...
run_doc_string = (f'molasses freq = {mol_freq_list}'
                  ...
                  f'probe att list = [0.1,1,2,3]'
                  f'pzt_para_list = [5.5,6,6.5,7,7.5,8,8.5,9,9.5,10]'
...
data_path_scope = daily_path / 'data' / run_name / 'cavity_scope'
...
for shot_num in range(num_shots):
    ...
    #     print([probe_avg_array[shot_num], trap_avg_array[shot_num]])
    #
    #     hf.close
    # except:
...
tweezer_00_vert_center_init = 53.35# starting position of tweezer 0 = 108MHz
tweezer_00_horiz_center_init = 15.8133
tweezer_horiz_span_init = 24
tweezer_vert_span_init = 16

tweezer_vert_spacing_init = 27.8*1 #1 is freq diff between neighboring tweezers on this run
tweezer_horiz_spacing_init = 1.3867
...
for point in range(num_points):
    ...
    pzt_counter = point // len(probe_att_list)
...
if save_fit_plot:
    ...
    for tweezer_num in range(num_tweezers):
        ...
        for mol_freq_counter in range(len(mol_freq_list)):
            ax3[tweezer_num, 0].plot(np.array(pzt_para_list), x_centerpos_frame[tweezer_num, :, mol_freq_counter],
                                     label=f"{mol_freq_list[mol_freq_counter]}")
            ax3[tweezer_num, 1].plot(np.array(pzt_para_list), x_radius_frame[tweezer_num, :, mol_freq_counter],
                                     label=f"{mol_freq_list[mol_freq_counter]}")
            ax3[tweezer_num, 2].plot(np.array(pzt_para_list), y_centerpos_frame[tweezer_num, :, mol_freq_counter],
                                     label=f"{mol_freq_list[mol_freq_counter]}")
            ax3[tweezer_num, 3].plot(np.array(pzt_para_list), y_radius_frame[tweezer_num, :, mol_freq_counter],
                                     label=f"{mol_freq_list[mol_freq_counter]}")
        for probe_att_counter in range(len(probe_att_list)):
            ax2[tweezer_num, 0].plot(np.array(pzt_para_list), x_centerpos_3[tweezer_num, :, probe_att_counter],
                                     label=f"{probe_att_list[probe_att_counter]}")
            ax2[tweezer_num, 1].plot(np.array(pzt_para_list), x_radius_3[tweezer_num, :, probe_att_counter],
                                     label=f"{probe_att_list[probe_att_counter]}")
            ax2[tweezer_num, 2].plot(np.array(pzt_para_list), y_centerpos_3[tweezer_num, :, probe_att_counter],
                                     label=f"{probe_att_list[probe_att_counter]}")
            ax2[tweezer_num, 3].plot(np.array(pzt_para_list), y_radius_3[tweezer_num, :, probe_att_counter],
                                     label=f"{probe_att_list[probe_att_counter]}")
            # for mol_freq_counter in range(len(mol_freq_list)):
            #     ax4[tweezer_num, 0].plot(np.array(pzt_para_list),
            #                              x_centerpos_4[tweezer_num, :, probe_att_counter, mol_freq_counter],
            #                              label=f"{probe_att_list[probe_att_counter]},mol_f={mol_freq_list[mol_freq_counter]}")
            #     ax4[tweezer_num, 1].plot(np.array(pzt_para_list),
            #                              x_radius_4[tweezer_num, :, probe_att_counter, mol_freq_counter],
            #                              label=f"{probe_att_list[probe_att_counter]},mol_f={mol_freq_list[mol_freq_counter]}")
            #     ax4[tweezer_num, 2].plot(np.array(pzt_para_list),
            #                              y_centerpos_4[tweezer_num, :, probe_att_counter, mol_freq_counter],
            #                              label=f"{probe_att_list[probe_att_counter]},mol_f={mol_freq_list[mol_freq_counter]}")
            #     ax4[tweezer_num, 3].plot(np.array(pzt_para_list),
            #                              y_radius_4[tweezer_num, :, probe_att_counter, mol_freq_counter],
            #                              label=f"{probe_att_list[probe_att_counter]},mol_f={mol_freq_list[mol_freq_counter]}")
    ...
    fig2.savefig('tweezers_postition_data_vs_probe_att', dpi=200)
...
for frame_num in range(num_frames):
    ...
    std_datafield = DataDictPointDataField(name=f'{frame_key}_std')
