...
high_na_datastream = DataStream(name='high NA Imaging', daily_path=daily_data_path, run_name=run_name,
                                file_prefix='jkam_capture')

###############################################################################################

datastream_name = 'High NA Imaging'
data_path_highNA = daily_data_path / 'data' / run_name / datastream_name
...
x_centerpos_2 = np.zeros((num_tweezers,len(tweezer_att_list)))
y_centerpos_2 = np.zeros((num_tweezers,len(tweezer_att_list)))
x_radius_2 = np.zeros((num_tweezers,len(tweezer_att_list)))
y_radius_2 = np.zeros((num_tweezers,len(tweezer_att_list)))
...
for point in range(num_points):
    ...
    pzt_counter = point // len(mol_att_list)
    for tweezer in range(num_tweezers):
        ...
        for frame_num in range(len(mol_freq_list)):
            ...
            ROI_array[tweezer,pzt_counter,mol_att_counter,frame_num] = ROI_temp
            ...
            x_centerpos_4[tweezer, pzt_counter, mol_att_counter,frame_num] = tweezer_00_horiz_center_init + tweezer * tweezer_horiz_spacing_init + x0 - tweezer_horiz_span_init / 2
            y_centerpos_4[tweezer, pzt_counter, mol_att_counter,frame_num] = tweezer_00_vert_center_init + tweezer * tweezer_vert_spacing_init + y0 - tweezer_vert_span_init / 2
            x_radius_4[tweezer, pzt_counter, mol_att_counter,frame_num] = sx
            y_radius_4[tweezer, pzt_counter, mol_att_counter,frame_num] = sy

            print(f"tweezer{tweezer_freq_list[tweezer]}, pzt_counter={pzt_counter}, probe_counter={mol_att_counter}, mol_counter={frame_num}"
                  f" - x0={x0:.03f},y0={y0:.03f},sx={sx:.03f},sy={sy:.03f}")

            ROI_init += photo[slice2d]
        ROI_init = ROI_init - np.amin(ROI_init)
        ROI_init_array[tweezer,pzt_counter,mol_att_counter] = ROI_init
        ...
        x_centerpos_3[tweezer,pzt_counter,mol_att_counter] = tweezer_00_horiz_center_init + tweezer*tweezer_horiz_spacing_init +  x0 - tweezer_horiz_span_init/2
        y_centerpos_3[tweezer,pzt_counter,mol_att_counter] = tweezer_00_vert_center_init + tweezer*tweezer_vert_spacing_init + y0 - tweezer_vert_span_init/2
        x_radius_3[tweezer,pzt_counter,mol_att_counter] = sx
        y_radius_3[tweezer,pzt_counter,mol_att_counter] = sy

        print(
            f"tweezer{tweezer_freq_list[tweezer]}, pzt_counter={pzt_counter}, probe_counter={mol_att_counter}"
            f" - x0={x0:.03f},y0={y0:.03f},sx={sx:.03f},sy={sy:.03f}")
if save_ROI_plot:
    fig.savefig('tweezers_raw_data',dpi=300)
for tweezer in range(num_tweezers):
    for pzt_counter in range(len(tweezer_att_list)):
...
if save_fit_plot:
    ...
    for tweezer_num in range(num_tweezers):
        ...
        for mol_freq_counter in range(len(mol_freq_list)):
            ...
            ax3[tweezer_num, 3].plot(np.array(tweezer_att_list), y_radius_frame[tweezer_num, :, mol_freq_counter],
                                     label=f"{mol_freq_list[mol_freq_counter]}")
        for mol_att_counter in range(len(mol_att_list)):
            ax2[tweezer_num, 0].plot(np.array(tweezer_att_list), x_centerpos_3[tweezer_num, :, mol_att_counter],
                                     label=f"{mol_att_list[mol_att_counter]}")
            ax2[tweezer_num, 1].plot(np.array(tweezer_att_list), x_radius_3[tweezer_num, :, mol_att_counter],
                                     label=f"{mol_att_list[mol_att_counter]}")
            ax2[tweezer_num, 2].plot(np.array(tweezer_att_list), y_centerpos_3[tweezer_num, :, mol_att_counter],
                                     label=f"{mol_att_list[mol_att_counter]}")
            ax2[tweezer_num, 3].plot(np.array(tweezer_att_list), y_radius_3[tweezer_num, :, mol_att_counter],
                                     label=f"{mol_att_list[mol_att_counter]}")
            # for mol_freq_counter in range(len(mol_freq_list)):
        ...
        ax2[tweezer_num, 0].plot(np.array(tweezer_att_list), x_centerpos_2[tweezer_num, :], label=f"sum", c='black')
        ax2[tweezer_num, 1].plot(np.array(tweezer_att_list), x_radius_2[tweezer_num, :], label=f"sum", c='black')
        ax2[tweezer_num, 2].plot(np.array(tweezer_att_list), y_centerpos_2[tweezer_num, :], label=f"sum", c='black')
        ax2[tweezer_num, 3].plot(np.array(tweezer_att_list), y_radius_2[tweezer_num, :], label=f"sum", c='black')
    ...
    fig2.savefig('tweezers_postition_data_vs_mol_att', dpi=200)
    fig3.savefig('tweezers_postition_data_vs_mol_freq', dpi=200)
