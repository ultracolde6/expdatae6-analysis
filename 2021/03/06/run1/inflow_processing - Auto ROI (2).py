...
for point in range(num_points):
    ...
    for tweezer in range(num_tweezers):
        ...
        if num_points>1:
            ...
            axs[tweezer].set_title(f"tweezer{tweezer}, point{point}", fontsize=10)
            axs[tweezer].imshow(ROI_init  , norm=colors.Normalize(vmin=np.amin(ROI_init), vmax=np.amax(ROI_init)), cmap='hot', interpolation='nearest')
            axs[tweezer].add_patch(rect)
