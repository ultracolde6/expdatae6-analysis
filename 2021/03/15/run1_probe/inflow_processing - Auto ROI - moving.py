...
from e6dataflow.processor import MultiCountsProcessor
    # , AvgStdProcessor, ThresholdProcessor
...
for point in range(num_points):
    ...
    for tweezer in range(num_tweezers):
        ...
        for pzt_counter in range(len(pzt_para_list)):
            ...
            ROI_array_avg[tweezer,pzt_counter] += ROI_temp
        ...
        print(
            ...
            f" - x0={x0:.03f},y0={y0:.03f},sx={sx:.03f},sy={sy:.03f}")
    if save_ROI_plot:
        fig.savefig(f'tweezers_raw_image_point{point}',dpi=60)
...
for tweezer in range(num_tweezers):
    ...
    for pzt_counter in range(len(pzt_para_list)):
        ...
        ROI_temp = ROI_array_avg[tweezer,pzt_counter]
        fit_struct = e6fit.fit_gaussian2d(ROI_temp, show_plot=False)
        x0, y0 = [fit_struct['x0']['val'], fit_struct['y0']['val']]
        sx, sy = [fit_struct['sx']['val'], fit_struct['sy']['val']]
        ...
        print(f"tweezer{tweezer_freq_list[tweezer]}, pzt_counter={pzt_counter}"
              f" - x0={x0:.03f},y0={y0:.03f},sx={sx:.03f},sy={sy:.03f}")
