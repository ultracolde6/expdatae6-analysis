...
if num_points > 1:
    fig, axs = plt.subplots(num_points, num_tweezers ,figsize=(30, 4))
else:
    fig, axs = plt.subplots(num_frames-2, num_tweezers ,figsize=(30, 4))
    ...
    for tweezer in range(num_tweezers):
        ...
        for frame_num in range(num_frames-2):
            ...
            if num_points > 1:
                ...
                axs[frame_num, tweezer].set_title(f"tweezer{tweezer}, mol_freq={mol_freq_list[frame_num]}", fontsize=10)
                axs[frame_num, tweezer].imshow(ROI_temp,
                                               norm=colors.Normalize(vmin=np.amin(ROI_temp), vmax=np.amax(ROI_temp)),
                                               cmap='hot', interpolation='nearest')
                axs[frame_num, tweezer].add_patch(rect)
...
for tweezer_num in range(num_tweezers):
    ...
    ax3[tweezer_num, 0].set_xlabel("mol_freq")
    ax3[tweezer_num, 1].set_xlabel("mol_freq")
    ax3[tweezer_num, 2].set_xlabel("mol_freq")
    ax3[tweezer_num, 3].set_xlabel("mol_freq")
    ...
    # for mol_freq_counter in range(len(mol_freq_list)):
    ax3[tweezer_num, 0].plot(np.array(mol_freq_list[:-1]), x_centerpos_frame[tweezer_num,0,:-1],\
                             label = f"span={np.amax(x_centerpos_frame[tweezer_num,0,:-1])-np.amin(x_centerpos_frame[tweezer_num,0,:-1]):.03f}")
    ax3[tweezer_num, 1].plot(np.array(mol_freq_list[:-1]), x_radius_frame[tweezer_num,0,:-1],\
                             label = f"span={np.amax(x_radius_frame[tweezer_num,0,:-1])-np.amin(x_radius_frame[tweezer_num,0,:-1]):.03f}")
    ax3[tweezer_num, 2].plot(np.array(mol_freq_list[:-1]), y_centerpos_frame[tweezer_num,0,:-1],\
                             label = f"span={np.amax(y_centerpos_frame[tweezer_num,0,:-1])-np.amin(y_centerpos_frame[tweezer_num,0,:-1]):.03f}")
    ax3[tweezer_num, 3].plot(np.array(mol_freq_list[:-1]), y_radius_frame[tweezer_num,0,:-1],\
                             label = f"span={np.amax(y_radius_frame[tweezer_num,0,:-1])-np.amin(y_radius_frame[tweezer_num,0,:-1]):.03f}")
    ...
    ax3[tweezer_num, 0].legend(title='span', bbox_to_anchor=(1.05, 1), loc='upper right')
    ax3[tweezer_num, 1].legend(title='span', bbox_to_anchor=(1.05, 1), loc='upper right')
    ax3[tweezer_num, 2].legend(title='span', bbox_to_anchor=(1.05, 1), loc='upper right')
    ax3[tweezer_num, 3].legend(title='span', bbox_to_anchor=(1.05, 1), loc='upper right')
